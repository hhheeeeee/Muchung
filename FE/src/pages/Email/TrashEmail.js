...
const TrashEmail = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/mails/delete`, {
        headers: {
  ...
  const handeleDelete = async () => {
    ...
    try {
      const response = await axios.patch(
        // `${BASE_URL}/mails/delete/${checkedItems}`,
  ...
  return (
    <>
      <div css={container}>
        <h2 className="header">
          <div>
            <span className="headertitle">휴지통</span>
            <Button
              variant="outlined"
              size="small"
              className="delete"
        ...
        >
          <thead>
            <tr className="field">
              ...
              <th style={{ padding: "0px 0px" }}>관심</th>
          ...
          <tbody>
            {rows.map((row) => (
              ...
              >
                ...
                <td style={{ padding: "0px 0px" }}>{row.title}</td>
              </tr>
            ))}
          </tbody>
        </Table>
